...
function normalizeNumberRecord<T extends object>(
  value: unknown,
  fallback: T,
): T {
...
export function migrateState(value: unknown): GameState | null {
  ...
  return {
    ...
    playerSettings: {
      ...
      fontScale:
        settings.fontScale === "small" || settings.fontScale === "large"
          ? settings.fontScale
          : "normal",
      theme: settings.theme === "neon" ? "neon" : "clear",
    ...
    lastSettlementDate: isString(value.lastSettlementDate)
      ...
      : dateKey(now),
    // This flag is session-scoped. Persisted saves must settle offline time again on load.
    initialized: false,
  };
}
