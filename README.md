...
- 一隻可命名的養成物與一個全幅霧面水族箱觀測區。
- 清透與夜光霓虹兩套主題，可調整字體大小、動態與對比。
- 最近反應統一顯示餵食回聲與觸碰回應，切換分頁後仍會保留。
- 身體冷暖色、透明度、柔軟度與流動速度會依長期餵食結果改變。
- 泡泡、像素星光與 2～5 根分節水母觸手會依型態強度出現。
- 8 種食物、12 種描述、6 種期待。
- 每次餵食產生 seeded random 結果，重新整理不會重擲既有紀錄。
- 重複相同食物與描述時套用遞減權重，不鼓勵刷取。
- 外觀會依長期訊號改變顏色、透明度、光澤、濕潤感與結構特徵。
- localStorage 自動儲存，保留最近 3 份內部備份，主存檔損壞時可自動復原。
- JSON 匯入會先驗證並補齊舊版欄位，錯誤檔案不會覆蓋目前進度。
